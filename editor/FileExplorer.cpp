...
void jed::FileExplorer::update_text_data()
{
    ...
    for (const auto& dirs : m_viewer.get_dirents()) {
        ...
        const std::string& path = std::get<std::string>(dirs);
...
const char* jed::FileExplorer::file_name(const std::string& full_path) const
{
